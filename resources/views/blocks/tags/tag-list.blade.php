@foreach($tags as $tag)
    <div id="tag-item-{{ $tag->id }}-{{ Utils::autoincrement('tags') }}" class="tag-item {{ count($tag->parents) > 1 ? 'moving-disable' : '' }}" draggable="true" data-tag-id="{{ $tag->id }}" data-parents-count="{{ count($tag->parents) }}">
        <div>
            {{ $tag->name }}
        ...
        <div class="tag-list">
            @include('blocks.tags.tag-list', ['tags' => $tag->children])
        </div>
    </div>
@endforeach